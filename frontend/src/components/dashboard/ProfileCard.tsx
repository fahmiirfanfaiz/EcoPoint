...
const ProfileCard: React.FC<ProfileCardProps> = ({ onOpenChallenges }) => {
  ...
  return (
    <div
      ...
      style={{
        boxShadow:
          "0px 2px 4px -1px rgba(0,0,0,0.03), 0px 4px 6px -1px rgba(0,0,0,0.05)",
      }}
    >
      {/* Decorative blurs */}
      <div
        className="pointer-events-none absolute -top-10 right-0 h-40 w-40 rounded-full opacity-50"
        style={{ background: "#D1FAE5", filter: "blur(20px)" }}
      />
      <div
        className="pointer-events-none absolute left-0 top-[70px] h-32 w-32 rounded-full opacity-50"
        style={{ background: "#FEF9C3", filter: "blur(20px)" }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 md:flex-row md:items-start">
        ...
        <div className="relative flex-shrink-0">
          ...
          >
            <img
              src={
                user
                  ? getAvatarUrl(user, user.profile_pic)
                  : getAvatarUrl({} as any, 0)
              }
              alt="Avatar"
          ...
          >
            ...
            <span className="font-quicksand whitespace-nowrap text-xs font-bold leading-4 text-white">
              {levelDisplay}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col gap-2 text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center gap-4 pb-2 md:justify-start">
            <span
              className="font-outfit rounded-lg bg-emerald-50 px-3 py-1 text-sm leading-5 text-gray-600"
              style={{ outline: "1px #D1FAE5 solid", outlineOffset: "-1px" }}
            >
              NIM: {user?.nim || "N/A"}
            </span>
            <span
          ...
          <div className="flex items-center gap-2 pb-1">
            <span className="font-outfit text-xs font-semibold text-amber-600">
              {levelName}
            </span>
            {nextLevel && (
              <span className="font-outfit text-xs text-gray-400">
                → {nextLevel.nama_level}
              </span>
            )}
          </div>
        ...
        <div className="flex min-w-[200px] flex-col gap-3">
          ...
          >
            <Plus size={20} />
            <span className="font-outfit text-base font-semibold leading-6">
              Lapor Sampah
            </span>
          </Link>
          <button
