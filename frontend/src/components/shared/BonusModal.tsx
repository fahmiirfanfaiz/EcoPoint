...
export default function BonusModal({ isOpen, onClose, bonusAmount }: BonusModalProps) {
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-[32px] bg-white p-8 text-center shadow-2xl animate-in fade-in zoom-in duration-300">
        ...
        <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-tr from-amber-400 to-yellow-300 shadow-xl shadow-amber-200 animate-bounce">
        ...
        <p className="font-quicksand text-sm text-gray-500 mb-8 px-4">
          Luar biasa! Kamu telah menyelesaikan seluruh Daily Challenges hari ini. Tambahan poin ini adalah bentuk apresiasi untuk konsistensi kamu.
        </p>

        <button
          onClick={onClose}
          className="w-full rounded-2xl bg-amber-500 py-3.5 font-bold text-white transition-transform hover:bg-amber-600 hover:scale-[1.02] active:scale-95 shadow-lg shadow-amber-200"
        >
          Klaim Bonus
        </button>
      </div>
