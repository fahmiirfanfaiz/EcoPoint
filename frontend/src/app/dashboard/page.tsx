...
export default function BerandaPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (dashRes.ok) {
        ...
        setStats(d.stats);
        setWeeklyActivity(d.weekly_activity ?? []);
      }
      if (chalRes.ok) {
  ...
  const completedCount = challenges.filter(
    (c) => c.user_progress?.is_completed,
  ).length;
    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-quicksand text-gray-400 font-semibold">
            Memuat beranda...
          </p>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-8 px-6 py-8">
      ...
      >
        ...
        />
        <div
          className="pointer-events-none absolute right-20 bottom-0 h-32 w-32 rounded-full"
          style={{ background: "rgba(255,255,255,0.05)" }}
        />

        <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="font-outfit text-emerald-100 text-sm mb-1">
              {greeting} 🌿
            </p>
            ...
            <p className="font-quicksand text-emerald-100 mt-2 max-w-md">
              Ayo mulai hari ini dengan aksi nyata untuk kampus yang lebih hijau
              dan bersih.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link
              href="/lapor-sampah"
              className="flex items-center gap-2 rounded-2xl bg-white px-6 py-3 font-outfit font-semibold text-emerald-700 shadow-lg transition-all hover:shadow-xl hover:scale-[1.02]"
            >
              <Plus size={20} /> Lapor Sampah
            </Link>
            <Link
              href="/tukar-poin"
              className="flex items-center gap-2 rounded-2xl bg-white/15 px-6 py-3 font-outfit font-semibold text-white backdrop-blur-sm transition-all hover:bg-white/25 border border-white/20"
            >
          ...
          <div className="relative z-10 mt-8 grid grid-cols-3 gap-4">
            {[
              ...
              },
              {
                icon: Recycle,
                label: "Laporan",
                value: stats.reports_submitted.toLocaleString(),
                accent: "bg-white/10 text-emerald-100",
              },
              {
                icon: () => (
                  <svg width="16" height="21" viewBox="0 0 16 21" fill="none">
                    <path
                      d="M8 0L9.79 3.58L13.71 4.15L10.85 6.94L11.59 10.87L8 9.01L4.41 10.87L5.15 6.94L2.29 4.15L6.21 3.58L8 0Z"
                      fill="currentColor"
                    />
                    <path
                      d="M4 12v8l4-2 4 2v-8"
                      fill="currentColor"
                      opacity="0.6"
                    />
                  </svg>
              ...
              >
                ...
                <div>
                  <p className="font-outfit text-[11px] text-emerald-200">
                    {s.label}
                  </p>
                  <p className="font-quicksand text-xl font-bold text-white">
                    {s.value}
                  </p>
                </div>
              </div>
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="font-nunito text-lg font-extrabold text-gray-900">
                    Challenge Hari Ini
                  </h2>
                  <p className="font-quicksand text-xs text-gray-400">
                    {completedCount}/{challenges.length} selesai · Total{" "}
                    {totalChalPoin} poin
                  </p>
                </div>
              </div>
            ...
            {challenges.length > 0 ? (
              <div className="space-y-3">
                {challenges.map((c) => {
                  const ci =
                    ...
                    CHALLENGE_ICONS.waste_report;
                  const Icon = ci.icon;
                  const progress = c.user_progress?.current_progress ?? 0;
                  const pct =
                    c.target_count > 0
                      ? Math.min((progress / c.target_count) * 100, 100)
                      : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`font-nunito text-sm font-bold text-gray-800 ${isFullyDone ? "line-through text-slate-500" : ""}`}
                        >
                          {c.nama_challenge}
                        </p>
                        <p className="font-quicksand text-xs text-gray-400 mt-0.5 truncate">
                          {c.deskripsi}
                        </p>
                        <div className="mt-2 flex items-center gap-2">
                          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-200">
                            <div
                              className={`h-full rounded-full transition-all duration-500 ${isFullyDone ? "bg-slate-400" : "bg-emerald-500"}`}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                          <span className="font-quicksand text-[11px] font-semibold text-gray-400">
                      ...
                      <div className="flex flex-col items-end gap-1 flex-shrink-0">
                        {!done && (
                          <>
                            <span className="font-quicksand text-sm font-bold text-emerald-600">
                              +{c.poin_hadiah}
                            </span>
                        ...
                        {done && !claimed && (
                          <button
                            onClick={() =>
                              handleClaim(c.challenge_of_the_day_id)
                            }
              ...
              <div className="flex flex-col items-center py-10 text-center">
                ...
                <p className="font-nunito text-sm font-bold text-gray-400">
            ...
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="font-quicksand text-xs text-gray-400">
                    {weeklyActivity.reduce((s, w) => s + w.count, 0)} laporan
                    minggu ini
                  </p>
                </div>
              </div>
              <span className="font-outfit rounded-lg bg-emerald-50 px-3 py-1 text-xs text-emerald-800">
                Minggu Ini
              </span>
            </div>
            <div
        ...
        <div className="flex flex-col gap-6">
          ...
          >
            <div className="flex items-center justify-between mb-5">
              ...
              <Link
                href="/leaderboard"
                className="font-quicksand text-xs font-bold text-emerald-600 hover:underline flex items-center gap-1"
              >
            ...
            <div className="space-y-2">
              {leaderboard.map((u, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex items-center gap-3 rounded-xl p-3 transition-colors ${i < 3 ? "bg-amber-50/50" : "hover:bg-gray-50"}`}
                  >
                    <span className="w-8 text-center font-nunito text-lg font-extrabold">
                      {i < 3 ? (
                        medals[i]
                      ) : (
                        <span className="text-sm text-gray-400">{u.rank}</span>
                      )}
                    </span>
                    <img
                      src={getAvatarUrl({ nama: u.nama } as any, u.profile_pic)}
                      alt=""
                      className="h-9 w-9 rounded-full bg-white flex-shrink-0 object-contain p-0.5"
                      style={{ border: "1px solid #e2e8f0" }}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="font-nunito text-sm font-bold text-gray-800 truncate">
                        {u.nama}
                      </p>
                      <p className="font-quicksand text-[11px] text-gray-400">
                        {u.reports_count} laporan
                      </p>
                    </div>
                    <span className="font-quicksand text-sm font-bold text-emerald-600 flex-shrink-0">
              ...
              {leaderboard.length === 0 && (
                <p className="font-quicksand text-sm text-gray-400 text-center py-6">
                  Belum ada data
                </p>
              )}
            </div>
          ...
          >
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-nunito text-base font-extrabold text-emerald-800">
                  Eco Tips
                </h3>
              </div>
              <p className="font-quicksand text-sm text-emerald-700 leading-relaxed">
                Tahukah kamu? Satu botol plastik bisa membutuhkan{" "}
                <strong>450 tahun</strong> untuk terurai secara alami. Dengan
                melaporkan dan mendaur ulang sampah plastik, kamu membantu
          ...
          {stats && (
            <div
              ...
              style={{ outline: "1px #ECFDF5 solid", outlineOffset: "-1px" }}
            >
              <h3 className="font-nunito text-base font-extrabold text-gray-900 mb-4">
                Dampak Kontribusimu 🌍
              </h3>
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="font-quicksand text-xs font-bold text-emerald-600">
                  ...
                  <div className="h-2 w-full overflow-hidden rounded-full bg-emerald-100">
                    <div
                      ...
                      style={{
                        width: `${Math.min(stats.reports_submitted * 2, 100)}%`,
                      }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="font-quicksand text-xs font-bold text-purple-600">
                      {stats.badges_earned}
                    </span>
                  </div>
                  <div className="h-2 w-full overflow-hidden rounded-full bg-purple-100">
                    <div
                      ...
                      style={{
                        width: `${Math.min(stats.badges_earned * 10, 100)}%`,
                      }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="font-quicksand text-xs font-bold text-amber-600">
                      {stats.total_poin.toLocaleString()}
                    </span>
                  </div>
                  <div className="h-2 w-full overflow-hidden rounded-full bg-amber-100">
                    <div
                      ...
                      style={{
                        width: `${Math.min(stats.total_poin / 50, 100)}%`,
                      }}
                    />
                  </div>
                </div>
